...
class ACC: public CongestionControl
{
protected:
	struct CongestionInformation *information;
	vector<ACCPacketInformation> pInfo;

public:
	ACC(int NID, PacketBuffer *Bf, Settings *S);
	~ACC();
...
private:
	double getPacketRTTL(int PID);
	double getProjNetGrowth(int PID);
	double getRiskMeanRTTL(int PID);
};
