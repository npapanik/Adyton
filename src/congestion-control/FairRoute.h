...
struct FairRoutePacketInformation
{
	int packetID;
	double senUtil;
	double recUtil;
...
class FairRoute: public CongestionControl
{
protected:
	struct CongestionInformation *information;
	vector<FairRoutePacketInformation> pInfo;

public:
	FairRoute(int NID, PacketBuffer *Bf, Settings *S);
	~FairRoute();
...
private:
	double getSenUtil(int PID);
	double getRecUtil(int PID);
};
