...
DropOldestASP::DropOldestASP(list<PacketEntry*> *B, Statistics *St): DroppingPolicy(B, St)
{
	return;
}


DropOldestASP::~DropOldestASP()
{
	return;
}


double DropOldestASP::Drop()
{
	bool found;
	double oldestTime;
	int drpHops;
	...
	{
		if(((*it)->CreationTime < oldestTime) && ((*it)->Hops > 0))
		{
			found = true;
