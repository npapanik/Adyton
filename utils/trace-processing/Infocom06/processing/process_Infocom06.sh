...
# How it works |
#---------------
# Step 1: Clean the dataset. Remove all external noise that is the set of devices with ID between 1 or 18 (according
# to the user's input) and 98. 
# 
...
if [ $# -ne 1 ]; then
	...
	echo "| options |";	
	echo "-----------";
	echo "<LR>: including Long Range Devices";
	echo "<LR>: excluding Long Range Devices";
    	exit 1;
fi
...
if [ "$1" == "LR" ];then
	...
	suffix="";
	echo -e "\n+---------------------------------------------------------------+";
	echo "| Processing Infocom06 dataset including the long range devices |";
	echo "+---------------------------------------------------------------+";
else
	minID=18;
	folder="SR";
	suffix="-SR";
	echo -e  "\n+---------------------------------------------------------------+";
	echo "| Processing Infocom06 dataset excluding the long range devices |";
	echo "+---------------------------------------------------------------+";
fi
...
maxID=98;
NN=$(echo "${maxID}-${minID}+1" | bc -l);
final=./${folder}/Infocom06${suffix}.txt;
input=../original/Exp6/contacts.Exp6.dat;
summary=./${folder}/summary.txt;
arrival=./${folder}/Infocom06${suffix}-presence.txt;
temp=./${folder}/tmp.txt;
active=./${folder}/active-nodes.txt;
inactive=./${folder}/inactive-nodes.txt;
dups=./${folder}/Infocom06-before-dups-removed.txt;
...
awk -v N=${maxID} -v minID=${minID} '{if($1 <= N && $2 <= N && $1 >= minID && $2 >= minID){printf("%d\t%d\t%d\t%d\n",$1-minID+1,$2-minID+1,$3,$4);}}' ${input} > ${final};
echo -e "${GREEN}[OK]${NC}";
