...
# How it works |
#---------------
# Step 1: Clean the dataset. Remove all external noise that is the set of devices with ID larger than 12.
# 
...
NN=12;
final=./HaggleCam.txt;
input=../original/Exp2/contacts.Exp2.dat;
summary=./summary.txt;
arrival=./HaggleCam-presence.txt;
temp=./tmp.txt;
active=./active-nodes.txt;
inactive=./inactive-nodes.txt;
dups=./Haggle-Cam-before-dups-removed.txt
rm -rf ${final};
rm -rf ${summary};
rm -rf ${arrival};
rm -rf ${temp};
