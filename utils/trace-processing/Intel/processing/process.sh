...
# How it works |
#---------------
# Step 1: Clean the dataset. Remove all external noise that is the set of devices with ID larger than 9.
# 
...
NN=9;
final=./Intel.txt;
input=../original/Exp1/contacts.Exp1.dat;
summary=./summary.txt;
arrival=./Intel-presence.txt;
temp=./tmp.txt;
active=./active-nodes.txt;
inactive=./inactive-nodes.txt;
dups=./Intel-before-dups-removed.txt
rm -rf ${final};
rm -rf ${summary};
rm -rf ${arrival};
rm -rf ${temp};
